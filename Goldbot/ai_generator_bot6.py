...
def generate_liquidity_flow_report(data: dict) -> str | None:
    ...
    try:
        ...
        # حساب أهداف السيولة (مناطق السحب)
        upper_target = round(r1 + (atr * 0.1), 2)  # منطقة فوق المقاومة بقليل لسحب الوقف
        lower_target = round(s1 - (atr * 0.1), 2)  # منطقة تحت الدعم بقليل لسحب الوقف

        # في حالة كان السعر قريباً جداً أو بعيداً جداً، نعتمد على ATR ليكون منطقياً لحظياً
        if upper_target <= gold: upper_target = round(gold + (atr * 0.4), 2)
        if lower_target >= gold: lower_target = round(gold - (atr * 0.4), 2)
...
- السعر الحالي: {gold}$
- هدف السيولة الصاعد (لسحب السيولة الشرائية): {upper_target}$
- هدف السيولة الهابط (لسحب السيولة البيعية): {lower_target}$
"""
...
🎯 أهداف السيولة ومناطق الجذب السعري:
▪️ مسار السيولة الصاعد: السعر يستهدف مستويات [الرقم العلوي]$ كهدف أولي لامتصاص السيولة الشرائية (ضرب الوقف).
▪️ مسار السيولة الهابط: السعر يستهدف مستويات [الرقم السفلي]$ كهدف أولي لامتصاص السيولة البيعية (ضرب الوقف).
