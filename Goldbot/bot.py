...
**📉 السيناريوهات (100%):**
   📈 صعود (X%): كسر {refs['above']}$ → الهدف [رقم]$ — [فوري (Spot)] أو [آجل (Futures)] — حدد واحداً فقط بناءً على السيولة
   📉 هبوط (Y%): كسر {refs['below']}$ → الهدف [رقم]$ — [فوري (Spot)] أو [آجل (Futures)] — حدد واحداً فقط بناءً على قوة الترند
   ⚡ تذبذب (Z%): النطاق [رقم]$-[رقم]$ والشرط — [X+Y+Z=100، كل واحد ≥ 15%]

🔴 قاعدة إضافية: في السيناريوهات، يجب أن تكتب "فوري (Spot)" أو "آجل (Futures)" صريحاً لكل سيناريو بشكل منفصل — ممنوع "أو كليهما" أو "أو الآجلة" — اختر الأنسب فقط\"\"\""
