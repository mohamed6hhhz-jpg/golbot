...
def _build_fedwatch_template(data: dict) -> str:
    ...
    try:
        ...
        # الفائدة الحالية (تقريبية) يمكن استنتاجها أو افتراضها من البيانات
        # في حال عدم وجودها، سنستخدم implied_rate كمرجع إذا كان منطقياً
        current_fed_rate = data.get('interest_rate', round(implied_rate * 4) / 4) # تقريب لأقرب 0.25
        
        # عدد مرات الخفض/الرفع المتوقعة على المدى القريب (كل خفض = 0.25% أي 25 نقطة أساس)
        rate_diff = current_fed_rate - implied_rate
        expected_cuts_raw = rate_diff / 0.25
        
        if expected_cuts_raw < -0.5:
            bias = "تشديد نقدي (تسعير لرفع الفائدة)"
            cuts_text = "رفع محتمل للفائدة"
            market_sentiment = "السوق يسعر احتمالية لرفع الفائدة استجابة لضغوط تضخمية مفاجئة."
        elif expected_cuts_raw <= 0.5:
            bias = "حياد إلى تيسير طفيف (تثبيت أو خفض واحد)"
            cuts_text = "0 - 25 نقطة أساس"
            market_sentiment = "استقرار نسبي مع احتمالات ضعيفة لخفض الفائدة قريباً (تريث الفيدرالي)."
        elif expected_cuts_raw < 1.5:
            bias = "تيسير نقدي معتدل (خفض مرتين)"
            cuts_text = "25 - 50 نقطة أساس"
            market_sentiment = "الأسواق تسعر بداية دورة تيسير نقدي هادئة ومدروسة لدعم النمو."
        elif expected_cuts_raw < 2.5:
            bias = "تيسير نقدي قوي (دورة خفض متوسطة)"
            cuts_text = "50 - 75 نقطة أساس"
            market_sentiment = "تسعير هجومي للأسواق يرجح تدخل الفيدرالي بقوة لحماية سوق العمل."
        else:
            bias = "تيسير نقدي عنيف (ركود أو أزمة)"
            cuts_text = "أكثر من 75 نقطة أساس"
            market_sentiment = "الأسواق تسعر تخفيضات طارئة أو دورة عنيفة جداً تحسباً لركود اقتصادي."
            
        # رؤية الأموال الذكية (مقارنة مع عوائد السندات لسنتين و 10 سنوات)
        twy = data.get('twy', current_fed_rate - 0.5)
        tnx = data.get('tnx', twy + 0.2)
        
        yield_curve_spread = tnx - twy
        if yield_curve_spread < -0.1:
            smart_money = f"انقلاب منحنى العائد ({yield_curve_spread:.2f}%): سوق السندات يحذر من ركود ويجبر الفيدرالي مستقبلاً على التخفيض."
        elif yield_curve_spread > 0.1:
            smart_money = f"منحنى عائد طبيعي ({yield_curve_spread:.2f}%): السيولة تتدفق للمخاطرة ولا يوجد تسعير ذعر في سوق السندات."
        else:
            smart_money = f"منحنى عائد مسطح ({yield_curve_spread:.2f}%): ترقب مؤسساتي وحالة عدم يقين للمسار القادم للفائدة."
            
        # مقارنة السنتين (التي تقيس توقعات الفائدة) مع الفائدة الفورية
        if twy < implied_rate - 0.3:
            bond_view = "عوائد السندات (2Y) تسعر وتيرة خفض أسرع بكثير من التسعير الفوري للعقود، مما يشير لرهانات قوية على التيسير."
        elif twy > implied_rate + 0.3:
            bond_view = "سوق السندات (2Y) حذر ولا يدعم التفاؤل المفرط بالتخفيضات الموجود في العقود الآجلة."
        else:
            bond_view = "تطابق قوي بين العقود الآجلة للفيدرالي وعوائد السندات (2Y)، مما يعطي مصداقية عالية للتسعير الحالي."

        template = (
            f"🏦 رادار الفيدرالي وتسعير الفائدة (FedWatch) 🇺🇸\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"🔹 الفائدة المعيارية التقريبية: {current_fed_rate:.2f}%\n"
            f"🔹 الفائدة المسعرة (عقود ZQ=F الفورية): {implied_rate:.2f}%\n"
            f"🔹 إجمالي الخفض المسعر: {cuts_text}\n"
            f"🔹 المسار العام: {bias}\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"📊 توجهات سيولة الأسواق (Market Sentiment):\n"
            f"   ▫️ {market_sentiment}\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"💡 رؤية الأموال الذكية (Smart Money):\n"
            f"   ▫️ {smart_money}\n"
            f"   ▫️ {bond_view}\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"🎯 تأثير التسعير الكلي على الذهب: {'إيجابي جداً 🟢 (داعم قوي للصعود)' if expected_cuts_raw >= 1.5 else 'إيجابي معتدل 🟢' if expected_cuts_raw > 0 else 'سلبي 🔴 (ضاغط للهبوط)'}"
        )
        return template
    except Exception as e:
...
def _build_fed_mindset_template(data: dict) -> str:
    ...
    if dxy > 104.5:
        employment_status = "سوق عمل ضيق واستهلاك قوي (يدعم الدولار ويخيف الفيدرالي من عودة التضخم)"
    elif dxy > 102.5:
        employment_status = "توازن في سوق العمل (نمو معتدل يعطي الفيدرالي مرونة في القرار)"
    else:
        employment_status = "تباطؤ في التوظيف والاقتصاد (يضغط على الفيدرالي لخفض الفائدة لحماية الاقتصاد)"
    ...
    if hawkish_points >= 3:
        ...
        powell_desc = "الترابط بين التضخم العنيد وقوة سوق العمل يضغط على الفيدرالي للإبقاء على الفائدة مرتفعة لفترة أطول."
        gold_impact = "ضغوط بيعية وتراجع للسيولة"
        dxy_impact = "يواصل قوته وهيمنته"
    elif hawkish_points <= 1:
        powell_tone = "تميل إلى التيسير (Dovish)"
        powell_desc = "تراجع التضخم مع ظهور علامات ضعف في التوظيف يعطي الفيدرالي الضوء الأخضر لبدء دورة خفض الفائدة."
        gold_impact = "انطلاقة قوية وتدفق سيولة شرائية"
        dxy_impact = "يفقد زخمه وجاذبيته"
    else:
        powell_tone = "حذرة وتعتمد على البيانات (Data Dependent)"
        powell_desc = "البيانات متضاربة والفيدرالي لن يأخذ قرارات متسرعة وسيلتزم بسياسة الانتظار."
        gold_impact = "تذبذب عرضي وانتظار للمحفز القادم"
        dxy_impact = "حركة عرضية ضمن نطاقات ضيقة"
        
    template = (
        f"🧠 **كيف يفكر الفيدرالي الآن؟ (تحليل الروابط الأربعة)**\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"الفيدرالي لا يتخذ قراره بناءً على مؤشر واحد، بل يعتمد على شبكة معقدة من 4 روابط رئيسية:\n\n"
        f"📊 **تحليل الروابط الأربعة بناءً على بيانات اللحظة:**\n\n"
        f"1️⃣ **التضخم الأساسي (الشبح الأكبر)**: {inflation:.1f}%\n"
        f"   ▪️ الهدف المقدس هو 2%. طالما التضخم أعلى من هذا الرقم، تظل يد الفيدرالي مقيدة.\n\n"
        f"2️⃣ **التوظيف (المستقرأ من قوة الدولار)**: {employment_status}\n"
        f"   ▪️ التوظيف القوي يرفع الاستهلاك ويعيد إشعال التضخم. الفيدرالي يتمنى رؤية تباطؤ ليتمكن من التخفيض بأمان.\n\n"
        f"3️⃣ **أسعار الطاقة (النفط)**: {oil:.2f}$ للبرميل\n"
        f"   ▪️ النفط هو المحرك الخفي لكل شيء. ارتفاعه يرفع أسعار السلع وينسف جهود الفيدرالي في محاربة التضخم.\n\n"
        f"4️⃣ **عوائد السندات (الترمومتر 10Y)**: {tnx:.2f}%\n"
        f"   ▪️ السندات تكشف الحقيقة. ارتفاع العوائد يعني تسعير تضخم أطول، بينما انخفاضها يعكس خوفاً من ركود قادم.\n\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"🔍 **بناءً على هذا الترابط الماكرو-اقتصادي، الصورة تتضح:**\n\n"
        f"▪️ **نبرة الفيدرالي المتوقعة**: **{powell_tone}**\n"
        f"   ↳ {powell_desc}\n\n"
        f"▪️ **مستقبل الدولار (DXY)**: **{dxy_impact}**\n"
        f"▪️ **تأثير ذلك على الذهب**: **{gold_impact}**\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"💡 فهم العلاقة بين هذه الروابط الأربعة يمنحك القراءة المسبقة لحركة الأسواق قبل حدوثها."
    )
    return template
