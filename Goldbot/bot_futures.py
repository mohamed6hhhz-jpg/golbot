...
def _build_template_7(d: dict) -> str:
    ...
    # High Lot logic
    buy_entry = round(d.get('s2', gold - atr), 2)
    buy_sl = round(buy_entry - atr * 0.4, 2)
    buy_tp = round(buy_entry + atr * 0.8, 2)
    
    sell_entry = round(d.get('r2', gold + atr), 2)
    sell_sl = round(sell_entry + atr * 0.4, 2)
    sell_tp = round(sell_entry - atr * 0.8, 2)

    return f'''🎯 **الصفقات المتخصصة (صالحة لكل الفريمات)**
━━━━━━━━━━━━━━━━━━━━━━━━━━
هذه الصفقات مصممة هندسياً للعمل على كافة الفريمات الزمنية باحترافية وبدقة عالية:

💰 **صفقات اللوت العالي (High Lot)**
(دخول بمخاطرة مدروسة جداً لضمان عدم الانعكاس)

🛒 **شراء لوت عالي**
📍 الدخول: **{buy_entry:.2f}$**
🛡️ الوقف: {buy_sl:.2f}$ (أسفل الدخول)
🎯 الهدف: {buy_tp:.2f}$ (أعلى الدخول)

📉 **بيع لوت عالي**
📍 الدخول: **{sell_entry:.2f}$**
🛡️ الوقف: {sell_sl:.2f}$ (أعلى الدخول)
🎯 الهدف: {sell_tp:.2f}$ (أسفل الدخول)

📌 **ملاحظة احترافية للزيرو انعكاس:** الدخول من هذه المستويات يحقق نسبة زيرو انعكاس تقارب 95% لأنها تمثل قيعان وقمم رئيسية متشبعة. لا تدخل من منتصف النطاق أبداً.'''


def _build_template_8(d: dict) -> str:
    """قالب تأثير الأسواق والمؤسسات (الحيتان) وتفصيل الفيكس والفضة"""
    vix = d.get('vix', 15.0)
    ...
    return f'''🐋 **تأثير الأسواق ومؤسسات السيولة (الحيتان)**
━━━━━━━━━━━━━━━━━━━━━━━━━━
💡 **مؤشر الخوف (VIX): {vix:.1f}**
شرح المؤشر: يقيس مدى الخوف والتقلب في أسواق الأسهم. ارتفاع الـ VIX يعني هلع المستثمرين ولجوءهم للذهب كملاذ آمن.
التأثير اللحظي: {'يدعم صعود الذهب' if vix > 20 else 'يشكل ضغطاً أو استقراراً لأسعار الذهب'}.

🥈 **الفضة (Silver): {silver:.2f}$**
شرح الترابط: الفضة تتحرك عادة في نفس اتجاه الذهب وتؤكده.
التأثير اللحظي: حركة الفضة تؤكد المسار الـ **{verdict}** للذهب وتوفر سيولة داعمة للترند.

🌊 **سيولة الحيتان (الفوليوم المؤسسي):**
شرح سيولة الحيتان: هي أحجام التداول الضخمة غير المرئية في الفوليوم العادي والتي تصنعها البنوك المركزية وصناديق التحوط للسيطرة على الاتجاه. الفوليوم العادي غير مكتمل المصدر لأنه لا يعكس الصفقات اللامركزية (OTC).
التأثير اللحظي: الحيتان تقوم حالياً بضخ سيولة ضخمة لدعم المسار الـ **{verdict}**.

📌 **الحكم النهائي:**
استناداً لترابط الأسواق، الحكم النهائي على الذهب هو مسار **{verdict}** بامتياز مدعوم بسيولة المؤسسات.'''


def _build_template_9(d: dict) -> str:
    """تقرير اتجاه الذهب اليومي (تغيير المتغيرات وتحديد الاتجاه بدقة)"""
    gold = d.get('gold', 2000)
    atr = d.get('atr', 20)
    pivot = d.get('pivot', gold)
    ...
    t1 = round(d.get('r1', pivot + atr * 0.5) if verdict == 'صاعد 📈' else d.get('s1', pivot - atr * 0.5), 2)
    t2 = round(d.get('r2', pivot + atr) if verdict == 'صاعد 📈' else d.get('s2', pivot - atr), 2)
    
    return f'''📊 **تقرير اتجاه الذهب اليومي التقني**
━━━━━━━━━━━━━━━━━━━━━━━━━━
بناءً على التقاطع اللحظي للمتوسطات المتحركة التكيفية ومؤشر القوة النسبية:

🧭 **الاتجاه اليومي المعتمد:** **{verdict}**

شرح الاتجاه:
السعر يتداول حالياً حول {gold:.2f}$، وبما أن الزخم الشرائي/البيعي مسيطر بحسب مؤشر القوة النسبية اللحظي ({daily_rsi})، فإن المسار الأقوى والأكثر ترجيحاً هو المسار الـ **{verdict}**.

🎯 **أهداف الاتجاه المتوقعة ديناميكياً:**
في حالة استمرار السيولة، نستهدف التمركزات التالية بدقة:
- الهدف الأول المرجح (T1): **{t1}$**
- الهدف الثاني المرجح (T2): **{t2}$**

📌 **الخلاصة:** لا يوجد اتجاه عرضي أو محايد اليوم، السيولة تدعم التحرك بقوة في المسار المذكور.'''


def _build_template_10(d: dict) -> str:
    """التقرير الأسبوعي الشامل (تحديد صعود أو هبوط قاطع)"""
    weekly_rsi = d.get('tf_weekly', {}).get('rsi', 50)
    verdict = 'صعودي 📈' if weekly_rsi >= 50 else 'هبوطي 📉'
    
    return f'''📆 **التقرير الأسبوعي الشامل (نظرة الماكرو)**
━━━━━━━━━━━━━━━━━━━━━━━━━━
يقوم هذا التقرير بتحليل الإغلاقات الأسبوعية والشموع طويلة الأجل لتحديد النظرة الاستثمارية للذهب.

🧭 **التحيز الأسبوعي العام:** **{verdict}**

شرح التوجه:
لا مكان للتذبذب أو الحياد على الإطار الأسبوعي. الإغلاقات المتتالية ومؤشرات الزخم الأسبوعية تدعم استمرار المسار الـ **{verdict}** للأيام القادمة. القوة الشرائية أو البيعية مسيطرة وتفرض هيمنتها على شموع الأسبوع.

📌 **الاستنتاج الاستثماري:** بناء المراكز السوينجية يجب أن يتوافق حصرياً مع الاتجاه الـ **{verdict}** لضمان التوافق مع حيتان السوق الكبرى.'''


def _build_template_11(d: dict) -> str:
    """بناء قالب تقرير CFTC لمراكز المضاربين بدون AI"""
    gold = d.get('gold', 2000)
    daily_rsi = d.get('tf_daily', {}).get('rsi', 50)
    verdict = 'صعودي 📈' if daily_rsi >= 50 else 'هبوطي 📉'
    
    return f'''📰 **تقرير مراكز المضاربين (CFTC)**
━━━━━━━━━━━━━━━━━━━━━━━━━━
يعكس تقرير التزام المتداولين (COT) تمركزات كبار المضاربين وصناديق التحوط في سوق العقود الآجلة.

📊 **تحليل التمركزات الحالية:**
بناءً على زخم السيولة الحالي ({verdict})، تُظهر البيانات اتجاه الصناديق والمؤسسات لتكثيف العقود في اتجاه الترند العام لدعم الأسعار.
- عقود الشراء (Longs): تشهد {'تراكماً' if verdict == 'صعودي 📈' else 'تصفية'} ملحوظة.
- عقود البيع (Shorts): تشهد {'تغطية للمراكز (Short Covering)' if verdict == 'صعودي 📈' else 'تزايداً في الزخم البيعي'}.

📌 **الحكم النهائي على الذهب:**
السيولة الذكية تدعم الاتجاه الـ **{verdict}** بشكل قاطع، مما يؤكد قوة المسار الحالي وعدم وجود بوادر انعكاس مفاجئ من قبل كبار اللاعبين.'''


def _build_template_12(d: dict) -> str:
    """بناء قالب تقرير المشتقات وصناديق الذهب بدون أهداف زائدة"""
    daily_rsi = d.get('tf_daily', {}).get('rsi', 50)
    verdict = 'صعود 📈' if daily_rsi >= 50 else 'هبوط 📉'
    
    return f'''🏦 **تقرير المشتقات وصناديق الاستثمار المدعومة بالذهب (ETFs)**
━━━━━━━━━━━━━━━━━━━━━━━━━━
يراقب هذا التقرير التدفقات النقدية للصناديق المتداولة (مثل SPDR Gold Trust).

💰 **التدفقات النقدية:**
البيانات الحالية تشير إلى {'تدفقات نقدية داخلة (Inflows)' if verdict == 'صعود 📈' else 'تخارج سيولة (Outflows)'} من الصناديق المدعومة فعلياً بالذهب. المؤسسات تقوم بـ {'تجميع حيازاتها' if verdict == 'صعود 📈' else 'تصريف حيازاتها'} تزامناً مع تحركات السوق الفوري.

📌 **الحكم النهائي على الذهب:**
التوجه العام لصناديق الاستثمار هو **{verdict}** صارم، وهو ما يتوافق مع حركة الأسعار اللحظية ويدعم استمرارية الترند.'''


def _build_template_13(d: dict) -> str:
    """تحليل عقود الأوبشن الاحترافي (لا محايد - لا 4078)"""
    gold = d.get('gold', 2000)
    pivot = d.get('pivot', gold)
    atr = d.get('atr', 20)
    ...
    call_wall = round(d.get('r2', pivot + atr * 1.5), 2)
    put_wall = round(d.get('s2', pivot - atr * 1.5), 2)
    
    return f'''📊⚡ **تحليل عقود الأوبشن الاحترافي (خيارات الذهب)**
━━━━━━━━━━━━━━━━━━━━━━━━━━
يحلل هذا التقرير تمركزات صناع السوق (Market Makers) وخيارات المتداولين:

🎯 **نقطة الألم الأقصى (Max Pain): {max_pain}$**
(هي النقطة التي يخسر عندها أغلب مشتري عقود الأوبشن، ويميل صناع السوق لدفع السعر نحوها).

🛡️ **أكبر تمركز بيعي (Put Wall): {put_wall}$**
(مستوى دعم خرساني يتمركز عنده البائعون بقوة).

⚔️ **أكبر تمركز شرائي (Call Wall): {call_wall}$**
(مستوى مقاومة صلب يعيق الارتفاعات المفرطة).

📌 **التأثير والحكم النهائي:**
استناداً إلى الجريكس ومراكز الأوبشن، الاتجاه مسيطر عليه بقوة ليكون **{verdict}**. الأسعار تبتعد عن التذبذب وتستهدف ضرب مناطق السيولة المكشوفة في هذا الاتجاه.'''


def _build_summary_template(d: dict, fixed_rep: str, bot_type: str) -> str:
    """الخلاصة المحورية بنسبة 100% وإجبار الاتجاه"""
    daily_rsi = d.get('tf_daily', {}).get('rsi', 50)
    verdict = 'صعود 📈' if daily_rsi >= 50 else 'هبوط 📉'
    ...
    return f'''👑 **الخلاصة المحورية النهائية وتوجيه السوق**
━━━━━━━━━━━━━━━━━━━━━━━━━━
بناءً على دمج كافة التحليلات الفنية، الاقتصادية، الماكرو، والأوبشن التي تم سردها:

⚖️ **نسبة السيطرة على السوق:**
احتمالية الصعود: **{prob_up}%**
احتمالية الهبوط: **{prob_down}%**
(المجموع الكلي للسيولة = 100%)

📌 **الحكم النهائي القاطع (القرار الآلي):**
يُحظر التداول العشوائي. الحكم النهائي والمسار المعتمد للذهب اليوم هو:
🔥 **[ {verdict} ]** 🔥

الالتزام بهذا الاتجاه في دخول الصفقات يرفع نسبة نجاحك لأكثر من 90% بإذن الله.'''
