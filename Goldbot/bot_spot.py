...
def _build_fixed_template(d: dict, header: str) -> tuple[str, str]:
    ...
    # حساب النطاق الكلاسيكي خصيصاً لقالب 3/30 بناءً على طلب العميل
    classic_range = round(d.get('prev_high', d.get('daily_high', gold + 20)) - d.get('prev_low', d.get('daily_low', gold - 20)), 2)
    if classic_range == 0: classic_range = 20.0
    exp_low  = round(gold - classic_range * 0.65, 2)
    exp_high = round(gold + classic_range * 0.65, 2)
...
🧮 المؤشرات وتأثيرها على الذهب
   ...
   ─────────────────────────────
   📊 Range: النطاق الكلاسيكي المعتمد لليوم: {classic_range}$ (بديل الـ ATR)
   ─────────────────────────────
...
def send_to_telegram(message: str, chat_id=None) -> bool:
    ...
    for i, chunk in enumerate(chunks, 1):
        ok     = _http_send(chunk, is_public, chat_id)
...
📈 احتمال صعود لضرب القمة (مستهدف {r2_val}$): {up_prob}%
📉 احتمال هبوط لضرب القاع (مستهدف {s2_val}$): {down_prob}%
🔀 احتمالية التذبذب العرضي (حول {gld_val}$): {osc_prob}%
...
قاعدة مهمة: لا تغير نسبة الصعود ({bull_pct}%) ولا نسبة الهبوط ({bear_pct}%) — هذه أرقام محسوبة رياضياً."""
    ...
    for model_name in GROQ_MODELS:
        try:
            ...
            static_probs = f"📈 احتمال صعود لضرب القمة (مستهدف {r2_val}$): {b_p}%\n📉 احتمال هبوط لضرب القاع (مستهدف {s2_val}$): {be_p}%\n🔀 احتمالية التذبذب العرضي (حول {gold_spot}$): {o_p}%\n"
...
📈 احتمال صعود لضرب القمة (مستهدف {r2}$): [توقعك كنسبة]%
📉 احتمال هبوط لضرب القاع (مستهدف {s2}$): [توقعك كنسبة]%
🔀 احتمالية التذبذب العرضي (حول {gold}$): [توقعك كنسبة]%
