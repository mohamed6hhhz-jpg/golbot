...
async def run_spot_v5():
    log.info("🚀 بدء تشغيل Goldbot V5 (Spot Core - Phase 2)...")
    ...
    if not data or 'spot_price' not in data:
        log.error("❌ فشل جلب البيانات للفوري.")
        return
        
    spot_price = data['spot_price']
    macro = data['macro']
    whales = data['spot_whales']
    
    tf_context = build_multi_timeframe_context(data['spot_dfs'])
    
    context = f"""
    -- البيانات الاقتصادية والحيتان --
    السعر الحالي: {spot_price}$
    ...
    for sec_id, sec_title in sections_to_generate:
        ...
        rep = generate_ai_section(sec_id, sec_title, context, is_spot=True)
        reports.append(rep)
        
    with open("test_reports_spot.txt", "w", encoding="utf-8") as f:
        f.write("\n" + "="*50 + "\n")
        for r in reports:
            f.write(r + "\n")
            f.write("="*50 + "\n")
    log.info("تم حفظ التقارير في test_reports_spot.txt")

if __name__ == "__main__":
    asyncio.run(run_spot_v5())
