...
def rewrite_spot_templates(filename):
    with open(filename, 'r', encoding='utf-8') as f:
        content = f.read()

    # The new code. We use python string literal for the patch.
    # We will use \n for literal newlines in the code we generate.
    # For actual \n that goes into strings in the python code (like "\n".join), we must use \\n.
    new_templates = """def _build_spot_s1(d: dict) -> str:
...
def _build_spot_s3(d: dict) -> str:
    ...
    content = re.sub(regex, new_templates + "\n\ndef _build_spot_s4(d: dict) -> str:", content, flags=re.DOTALL)

    with open(filename, 'w', encoding='utf-8') as f:
        f.write(content)

rewrite_spot_templates('c:/Users/lenovo/Desktop/alltoools/Goldbot/bot_spot.py')
