...
with open('s_nums_code.txt', 'r', encoding='utf-8') as f:
    s_nums_code = "\n" + f.read() + "\n"
...
def patch_file(filepath):
    ...
    if "def _build_liquidity_time_targets(" not in text:
        text = text.replace("def send_reports(", get_liquidity_targets_code() + "\ndef send_reports(")
        changed = True

    if "def _s_nums(" not in text:
        text = text.replace("def send_reports(", s_nums_code + "\ndef send_reports(")
        changed = True
